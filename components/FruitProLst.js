...
class FruitProLst extends Component {
    constructor(props) {
        super(props);
    }
    render() {
        ...
        const product_lst = !!products_arr && products_arr.length  ? (products_arr.filter((prod) => {
            ...
            if(initial_products_id.indexOf(id.toString()) != -1){
                const {product_id, quantity , price_incl_tax} = initial_products[id];
                prod['is_product_added_to_basket'] = true;
                prod['product_added_to_basket_qty'] = quantity;
            }else{
                prod['is_product_added_to_basket'] = false;
                prod['product_added_to_basket_qty'] = "";
            }
            if(product_class === "fruits"){
                return prod
            }    
            })
        ):null;   
...
const mapStateToProps = (state) => {
    ...
    console.log('This sis the products id array', products_id);
